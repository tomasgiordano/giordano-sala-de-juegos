...
export class LFigura extends Monocluster{
  ...
  public constructor(x : number,y : number,width : number,height : number){
    ...
    this.mono[0] = new MonoSprite( x, y, width, height,"orange");
    this.mono[1] = new MonoSprite( x, y + 30, width, height,"orange");
    this.mono[2] = new MonoSprite( x, y + 60, width, height,"orange");
    this.mono[3] = new MonoSprite( x + 30, y + 60, width, height,"orange");
  }
  public RotacionHitbox(logic : TetrisLogic){
    ...
    if(!flagDown){
      ...
      {
        if(this.extHitboxRotationL(logic) == false){
          if(flagLeft && this.nRotacion == 0){
            this.mono.forEach(element=>{
              element.SetX(30);
            });
          }else if(flagRight && this.nRotacion == 2){
            this.mono.forEach(element=>{
              element.SetX(-30);
            });
  ...
  public extHitboxRotationL(logic : TetrisLogic){
    let aux = false;
    let aux1 = false;
    let aux2 = false;
    if(this.nRotacion == 0){
